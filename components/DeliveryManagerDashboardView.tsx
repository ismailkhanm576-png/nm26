...
export const DeliveryManagerDashboardView: React.FC<{ 
    ...
    const openingData = [94, 95, 93, 96, 97, 98].map(v => selectedCity === '全省' ? v : Math.min(100, Math.max(85, v + (citySeed - 2.5))));
    const evaluationData = [90, 92, 91, 93, 94, 95].map(v => selectedCity === '全省' ? v : Math.min(100, Math.max(80, v + (citySeed - 3))));
    const deliveryData = selectedCity === '全省' ? [800, 950, 850, 1100, 1200, 1300] : [80, 95, 85, 110, 120, 130].map(v => Math.floor(v * citySeed));
    const dispatchData = [96, 97, 95, 98, 98.5, 99].map(v => selectedCity === '全省' ? v : Math.min(100, Math.max(90, v + (citySeed - 1.5))));
    const acceptanceTrendData = [92, 94, 93, 95, 96, 97].map(v => selectedCity === '全省' ? v : Math.min(100, Math.max(85, v + (citySeed - 2))));
    const completionTrendData = [90, 93, 92, 94, 95, 96].map(v => selectedCity === '全省' ? v : Math.min(100, Math.max(80, v + (citySeed - 3))));
    ...
    return (
        <div className="relative flex-1 flex flex-col lg:flex-row overflow-hidden bg-transparent border border-blue-500/30 shadow-[inset_0_0_20px_rgba(0,133,208,0.1)] text-white p-4 gap-4">
                ...
                <div className="w-full lg:h-[58%] flex flex-col lg:flex-row gap-4 min-h-0 shrink-0">
                    <div className="flex-1 bg-[#0b1730]/30 border border-blue-500/20 p-4 rounded-sm shadow-[0_0_15px_rgba(0,210,255,0.1)] flex flex-col relative min-w-0">
                        <div className="text-sm font-bold text-white mb-2 flex items-center justify-between shrink-0">
                            ...
                            {selectedCity !== '全省' && <button className="text-xs text-blue-300 hover:text-neon-blue transition-colors cursor-pointer" onClick={() => setSelectedCity('全省')}>返回全省</button>}
                        </div>
                    ...
                    <div className="flex-1 overflow-hidden min-h-0">
                        <table className="w-full text-left text-xs text-blue-100 whitespace-nowrap">
                            <thead className="sticky top-0 bg-[#0b1730] z-10">
                                <tr>
                                    <th className="p-2 border-b border-blue-500/30">级别</th>
