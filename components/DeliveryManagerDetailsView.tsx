...
export const DeliveryManagerDetailsView: React.FC = () => {
    ...
    }, [searchCity, searchDistrict, searchName]);
    ...
    const handleExport = () => {
        const headers = ['姓名', '地市', '区县', '得分', '开通及时率', '用后即评', '交付量', '分派及时率', '报结及时率', '受理及时率'];
        const csvContent = [
            headers.join(','),
            ...filteredData.map(row => 
                [row.name, row.city, row.district, row.score, row.openingRate, row.evaluation, row.delivery, row.dispatchRate, row.completionRate, row.acceptanceRate].join(',')
            )
        ].join('\n');
            ...
            <div className="bg-transparent p-3 border-b border-blue-500/20 flex flex-wrap items-center justify-between gap-y-3 gap-x-6 shrink-0">
                <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <StyledButton variant="outline" onClick={() => { setSearchCity(''); setSearchDistrict(''); setSearchName(''); setCurrentPage(1); }} icon={<RefreshCwIcon className="w-3.5 h-3.5" />}>重置</StyledButton>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar min-h-0 relative">
                <table className="w-full text-left text-sm text-blue-100 whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody className="text-blue-100 text-sm">
                        ...
                        )) : <tr><td colSpan={10} className="p-8 text-center text-blue-300/50 border-b border-blue-500/10">暂无数据</td></tr>}
                    </tbody>
                </table>
            </div>
