...
export const PortraitDetailsView: React.FC = () => {
    ...
    const handleExport = () => {
        const headers = ['姓名', '班组', '地市', '区县', '得分', '装机量', '及时率', '满意度', '质检通过率'];
        const csvContent = [
            headers.join(','),
            ...filteredData.map(row => 
                [row.name, row.team, row.city, row.district, row.score, row.volume, row.timeliness, row.satisfaction, row.quality].join(',')
            )
        ].join('\n');
                ...
                <table className="w-full text-left text-sm text-blue-100 whitespace-nowrap border-separate border-spacing-0">
                    ...
                    <tbody className="text-blue-100 text-sm">
                        {paginatedData.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={9} className="p-8 text-center text-blue-300/50 border-b border-blue-500/10">
                                    暂无数据
                                </td>
                            </tr>
                        )}
